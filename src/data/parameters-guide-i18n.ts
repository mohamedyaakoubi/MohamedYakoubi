...
const en: ParametersGuideI18n = {
  ...
  scoringFlags: {
    ...
    flags: [
      ...
      { name: 'enableComposite', default: 'true', what: 'Averages the per-metric grades (1–5 scale) of enabled metrics that returned a non-null value. CER, Transcript CER, WER, Transcript WER, SegER, SER, and Transcript SER contribute — SACR is excluded because many datasets lack a speaker column, making SACR-inclusive composites incomparable across batches.', whenToDisable: 'When your system reads individual metric fields directly and does not display an aggregate grade.' },
      ...
      { name: 'transcriptCerInComposite', default: 'true', what: 'Include Transcript CER in the composite grade calculation. When false, transcriptCER is still computed and returned but does not affect the composite score.', whenToDisable: 'When transcript-column CER is a reference metric only and you want the composite to reflect the other metrics.' },
      { name: 'transcriptWerInComposite', default: 'true', what: 'Include Transcript WER in the composite grade calculation. When false, transcriptWER is still computed but excluded from the composite average.', whenToDisable: 'When transcript-column WER is tracked for reference but should not influence the composite grade.' },
      { name: 'transcriptSerInComposite', default: 'true', what: 'Include Transcript SER in the composite grade calculation. When false, transcriptSER is computed but used as a standalone sentence-level diagnostic without affecting the composite.', whenToDisable: 'When paragraph-level sentence churn is a secondary diagnostic signal and you want CER/WER/SegER/SER to determine the composite.' },
    ],
    compositeNote:
...
const fr: ParametersGuideI18n = {
  ...
  scoringFlags: {
    ...
    flags: [
      ...
      { name: 'transcriptCerInComposite', default: 'true', what: 'Inclure le CER de transcription dans le calcul de la note composite. Quand false, transcriptCER est toujours calculé et renvoyé mais n\'affecte pas le composite.', whenToDisable: 'Quand le CER de la colonne transcript est une métrique de référence uniquement et que vous voulez que le composite reflète les autres métriques.' },
      { name: 'transcriptWerInComposite', default: 'true', what: 'Inclure le WER de transcription dans le calcul de la note composite. Quand false, transcriptWER est calculé mais exclu de la moyenne composite.', whenToDisable: 'Quand le WER de la colonne transcript est suivi comme référence mais ne doit pas influencer la note composite.' },
      { name: 'transcriptSerInComposite', default: 'true', what: 'Inclure le SER de transcription dans le calcul de la note composite. Quand false, transcriptSER est calculé mais utilisé comme signal diagnostique de phrase sans affecter le composite.', whenToDisable: 'Quand le taux de renouvellement de phrases est un signal secondaire et que vous voulez que CER/WER/SegER/SER déterminent le composite.' },
    ],
    compositeNote:
...
const ar: ParametersGuideI18n = {
  ...
  enableInlineDiff: {
    ...
    flags: [
      ...
      { name: 'transcriptCerInComposite', default: 'true', what: 'تضمين CER النص في حساب الدرجة المركبة. عند false، يُحسب transcriptCER ويُعاد لكنه لا يؤثر في المركّب.', whenToDisable: 'عندما يكون CER عمود النص مقياساً مرجعياً فقط وتريد أن يعكس المركّب المقاييس الأخرى.' },
      { name: 'transcriptWerInComposite', default: 'true', what: 'تضمين WER النص في حساب الدرجة المركبة. عند false، يُحسب transcriptWER لكن يُستبعد من متوسط المركّب.', whenToDisable: 'عندما يُتتبَّع WER عمود النص كمرجع لكن لا يجب أن يؤثر في الدرجة المركبة.' },
      { name: 'transcriptSerInComposite', default: 'true', what: 'تضمين SER النص في حساب الدرجة المركبة. عند false، يُحسب transcriptSER لكن يُستخدم كإشارة تشخيصية على مستوى الجملة دون التأثير في المركّب.', whenToDisable: 'عندما يكون معدل تغيير الجمل إشارة ثانوية وتريد أن تحدد CER/WER/SegER/SER درجة المركّب.' },
    ],
    compositeNote:
