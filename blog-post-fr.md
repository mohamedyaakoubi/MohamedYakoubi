# Quand l'IA Construit la Serrure et Oublie la Clé

*Une histoire de curiosité, d'une campagne de jeu, de confiance aveugle accordée au client, et de ce qui arrive quand le développement assisté par IA est mis en production sans regard sur la sécurité — analysé avec l'aide de l'IA elle-même.*

---

## Préface

Ce post documente une analyse de sécurité que j'ai conduite sur une campagne de jeu accessible publiquement. Aucun système n'a été endommagé. Aucune donnée utilisateur n'a été consultée ou extraite. Aucune identité n'est divulguée. Tous les tests ont été réalisés sur des comptes que je contrôlais. Les résultats ont été signalés au développeur en privé avant cette publication.

Ce qui rendait cette analyse particulièrement intéressante : l'application avait été construite en grande partie avec l'assistance de l'IA. Et cette analyse elle-même — la méthodologie, les scripts, l'énumération — a également été conduite avec l'assistance de l'IA. L'ironie ne m'échappe pas, et elle est au cœur de ce post.
...
Quand une application loggue aussi agressivement en production — sur chaque service, chaque page, chaque action utilisateur — cela vous dit l'une de deux choses : elle a été construite à la hâte, ou elle a été construite avec l'assistance de l'IA et les scaffoldings de debug n'ont jamais été retirés. Dans ce cas, les preuves pointaient vers les deux.

Cela révélait aussi quelque chose sur l'hypothèse de déploiement. Le développeur avait distribué l'app dans un navigateur mobile intégré, où la plupart des utilisateurs n'ouvriraient jamais une console. Si les utilisateurs ne peuvent pas voir la console, les logs n'ont pas d'importance. Ce raisonnement, conscient ou non, est exactement le genre d'hypothèse qui mène à shipper des logs de debug en production.
...
CLIENT (Application CSR navigateur)
  ...
  ├── Signature HMAC ─────────── clé secrète retournée par le serveur ⚠️
...
AUTH
  ├── POST /auth/otp/request ── limité côté serveur ✓
  └── POST /auth/verify ── compteur de tentatives cumulatif ✓
```
...
Comme décrit en introduction — les logs console n'étaient pas accidentels. Ils étaient omniprésents. Chaque service majeur, chaque action utilisateur, chaque événement d'authentification loggait dans la console du navigateur en production.
...
Chaque résultat critique partage la même cause racine : le serveur faisait confiance au client.
...
Client                          Serveur
  |                               |
  |── Créer session ─────────────>|
  |<── { sessionId } ─────────────|  Pas de secret. Pas de clé de signature.
  |                               |
  |── "Générer résultat" ─────────>|  Juste un déclencheur.
  |<── { input1, input2, score } ──|  Le serveur génère. Le serveur calcule le score.
  |                               |
  |── "Soumettre manche" ─────────>|  Pas de valeurs. Le serveur sait déjà.
  |<── { roundResult } ────────────|
```
...
3. **Supprimer les sorties de debug avant de shipper.** Mettre en place une étape de build qui supprime ou désactive tous les appels console.log en production. Un navigateur mobile intégré ne rend pas les logs console invisibles — il les rend juste moins pratiques à consulter.
...
L'objectif de ce post n'est pas d'embarrasser qui que ce soit. Construire une plateforme de jeu full-stack, seul, avec une deadline, est genuinement difficile. Le développeur a shippé quelque chose avec lequel de vrais utilisateurs se sont engagés. Ce n'est pas rien.
