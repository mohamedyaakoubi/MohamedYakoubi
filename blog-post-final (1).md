...
*A story about curiosity, a game campaign, client-side trust, and what happens when AI-assisted development ships without a security lens — analyzed with AI assistance itself.*

---

## Preface

This post documents a security analysis I conducted on a publicly accessible game campaign. No systems were damaged. No user data was accessed or extracted. No identities are disclosed. All testing was performed on accounts I controlled. Findings were reported to the developer privately before this publication.

What made this particularly interesting: the application was built largely with AI-assisted coding. And this analysis itself — the methodology, the scripts, the enumeration — was also conducted with AI assistance. That irony is not lost on me, and it is a core part of what this post is about.
...
CLIENT (CSR Browser App)
  ...
  └── Console logs (production) ───── session internals exposed ⚠️

API (REST Backend)
  │
  ├── POST /session/create
  │         creates session, returns signing secret ⚠️
  ├── POST /session/:id/submit-round
  │         accepts outcome values from client ⚠️
  ├── POST /session/:id/submit-result
  │         accepts winner determination from client ⚠️
```
...
Client                          Server
  |                               |
  |── Create session ───────────> |
  |<── { sessionId } ─────────────|  No secret. No signing key.
  |                               |
  |── "Generate outcome" ────────> |  Just a trigger.
  |<── { input1, input2, score } ──|  Server generates. Server scores.
  |                               |
  |── "Submit round" ───────────> |  No values. Server already knows.
  |<── { roundResult } ───────────|
```
...
#PenTesting #AppSec #AICoding #WebSecurity #SecureByDesign #ResponsibleDisclosure #GameSecurity #ClientSideTrust
